...
Single-image and directory CLIs were unified: individual methods accept one
image, several images, or a directory. Saved transform state moved from `.npy`
to stamped `.npz` (old `.npy` files are not loadable — refit them).
...
- **population methods**: `fit` returns a fitted transform —
  `tx = inorm.nyul.fit(imgs); tx(new_img); tx.save("x.npz")`
...
- `pymedio` images are gone; pass numpy arrays or nibabel images and get the
  same type back.
- `scikit-fuzzy` is gone; fuzzy c-means is implemented in-house (seeded,
  deterministic by default).
- `Modality`/`TissueType` enums became plain validated strings (`"t1"`, `"wm"`).
- The `normalize_image(method="...")` dispatcher (v3) was removed; call the
  method by name.
- antspy/matplotlib are optional extras as before, now imported lazily:
  `[ants]`, `[plot]`.
