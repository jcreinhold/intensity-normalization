...
def arg_parser():
    parser = argparse.ArgumentParser(
        description="Use FCM to model the tissue classes of the brain and use the "
...
def process(image_fn, brain_mask_fn, tissue_mask_fn, output_dir, args, logger):
    ...
    if brain_mask_fn is not None:
        ...
        tissue_mask = fcm.find_tissue_mask(
            image, mask, threshold=args.threshold, tissue_type=args.tissue_type
        )
    ...
    if tissue_mask_fn is not None:
        ...
        normalized = fcm.fcm_normalize(image, tissue_mask, args.norm_value)
...
def main(args=None):
    ...
    try:
        if not args.single_image:
            ...
            mask_fns = (
                io.glob_nii(args.brain_mask)
                if args.brain_mask is not None
                else [None] * len(img_fns)
            )
            ...
            if args.modality.lower() == "t1" and args.tissue_mask is None:
                ...
                if os.path.exists(tissue_mask_dir):
                    ...
                    logger.info(
                        "Creating tissue mask directory: {}".format(tissue_mask_dir)
                    )
                ...
                for i, (img, mask) in enumerate(zip(img_fns, mask_fns), 1):
                    ...
                    logger.debug(
                        "Tissue mask {} ({:d}/{:d})".format(mask_base, i, len(img_fns))
                    )
            ...
            for i, (img, tissue_mask) in enumerate(zip(img_fns, tissue_masks), 1):
                ...
                logger.info(
                    "Normalizing image {} ({:d}/{:d})".format(base, i, len(img_fns))
                )
                logger.debug(
                    "Tissue mask {} ({:d}/{:d})".format(tissue_base, i, len(img_fns))
                )
            ...
            if args.tissue_mask is None and args.modality.lower() == "t1":
                logger.info("Creating tissue mask for {}".format(args.image))
                process(
                    args.image, args.brain_mask, None, args.output_dir, args, logger
                )
