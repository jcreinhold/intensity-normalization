...
def arg_parser():
    parser = argparse.ArgumentParser(
        description="coregister a set of MR images (e.g., to MNI or to the T1 image)"
    )
    ...
    options.add_argument(
        "--orientation", type=str, default="RAI", help="output orientation of imgs"
    )
    options.add_argument(
        "-r",
        "--registration",
...
def main(args=None):
    ...
    try:
        ...
        img_fns = glob_nii(args.image_dir)
        if not os.path.exists(args.output_dir):
            logger.info("Making Output Directory: {}".format(args.output_dir))
            os.mkdir(args.output_dir)
        if (
            not os.path.exists(os.path.join(args.output_dir, "tfms"))
            and args.transform_dir is None
        ):
        ...
        if args.template_dir is None:
            ...
            template = ants.image_read(ants.get_ants_data("mni")).reorient_image2(
                args.orientation
            )
            ...
            if len(template_fns) != len(img_fns):
                raise NormalizationError(
                    "If template images are provided, they must be in "
                    "correspondence (i.e., equal number) with the source images"
                )
        if args.transform_dir is not None:
            tfm_fns = sorted(glob(os.path.join(args.transform_dir, "*.mat")))
            if len(tfm_fns) != len(img_fns):
                raise NormalizationError(
                    "If transforms are provided, they must be in "
                    "correspondence (i.e., equal number) with the source images"
                )
        for i, img in enumerate(img_fns):
            _, base, _ = split_filename(img)
            logger.info(
                "Registering image to template: {} ({:d}/{:d})".format(
                    base, i + 1, len(img_fns)
                )
            )
            if args.template_dir is not None:
                template = ants.image_read(template_fns[i])  # noqa
                orientation = (
                    template.orientation if hasattr(template, "orientation") else None
                )
            input_img = ants.image_read(img)
            input_img = (
                input_img.reorient_image2(orientation)
                if orientation is not None  # noqa
                else input_img
            )
            if args.transform_dir is None:
                if not args.no_rigid:
                    logger.info(
                        "Starting rigid registration: {} ({:d}/{:d})".format(
                            base, i + 1, len(img_fns)
                        )
                    )
                    ...
                    tx = None
                logger.info(
                    "Starting {} registration: {} ({:d}/{:d})".format(
                        args.registration, base, i + 1, len(img_fns)
                    )
                )
                mytx = ants.registration(
                    fixed=template,
                    ...
                    type_of_transform=args.registration,
                )
            else:
                logger.info(
                    "Loading transform: {} ({:d}/{:d})".format(
                        tfm_fns[i], i + 1, len(img_fns)  # noqa
                    )
                )
            ...
            if args.transform_dir is None:
                for j, tfm in enumerate(mytx["fwdtransforms"]):
                    tfm_fn = os.path.join(
                        args.output_dir, "tfms", base + "_{}.mat".format(j)
                    )
