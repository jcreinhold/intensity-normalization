...
def arg_parser():
    parser = argparse.ArgumentParser(
        description="Create a tissue mask for a given brain (i.e., label CSF/GM/WM)"
    )
...
def main(args=None):
    ...
    try:
        ...
        for i, (img_fn, mask_fn) in enumerate(zip(img_fns, mask_fns), 1):
            ...
            logger.info(
                "Creating Mask for Image: {}, ({:d}/{:d})".format(base, i, len(img_fns))
            )
            img = io.open_nii(img_fn)
            mask = io.open_nii(mask_fn)
            tm = (
                fcm_class_mask(img, mask, hard_seg)
                if not args.gmm
                else gmm_class_mask(img, mask, "t1", False, hard_seg)
            )
