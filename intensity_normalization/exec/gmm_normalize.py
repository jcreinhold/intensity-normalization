...
def arg_parser():
    parser = argparse.ArgumentParser(
        description="Use GMM to model the tissue classes in brain and "
    ...
    options.add_argument(
        "-bm", "--background-mask", type=str, help="path to a mask of the background"
    )
    options.add_argument(
        "-w",
        "--wm-mean",
    ...
    options.add_argument(
        ...
        type=float,
        default=1,
        help="value by which to set the WM mean to",
    )
    options.add_argument(
        "-s",
        "--single-image",
    ...
    options.add_argument(
        ...
        action="store_true",
        default=False,
        help="store the found WM mean, uses wm-mean as the name if true",
    )
    options.add_argument(
...
def process(image, brain_mask, args, logger):
    ...
    if args.wm_mean is not None:
        logger.info("Loading WM peak: ", args.wm_mean)
        peak = float(np.load(args.wm_mean))
    else:
        peak = gmm_class_mask(img, brain_mask=mask, modality=args.modality)
        if args.save_wm_mean:
            np.save(os.path.join(dirname, base + "_wmpeak.npy"), peak)
    normalized = gmm.gmm_normalize(
        img, mask, args.norm_value, args.modality, args.background_mask, peak
    )
