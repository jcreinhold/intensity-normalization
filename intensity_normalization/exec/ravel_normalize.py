...
def arg_parser():
    parser = argparse.ArgumentParser(
        description="Use RAVEL to normalize a directory of NIfTI MR images of the brain."
    )
...
def main(args=None):
    ...
    try:
        ...
        img_fns = io.glob_nii(args.image_dir)
        mask_fns = io.glob_nii(args.brain_mask_dir)
        if len(img_fns) != len(mask_fns) or len(img_fns) == 0:
            raise NormalizationError(
                "Image directory ({}) and mask directory ({}) must contain the same "
                "(positive) number of images!".format(
                    args.image_dir, args.brain_mask_dir
                )
            )
        ...
        V = ravel.image_matrix(img_fns, args.modality, masks=mask_fns)
        ...
        if args.plot_histograms:
            ...
            ax = all_hists(output_dir, args.brain_mask_dir)
            ax.set_title("RAVEL")
