...
def csf_mask(
    ...
    Args:
        image (ants.core.ants_image.ANTsImage or nibabel.nifti1.Nifti1Image): target img
    ...
    if hasattr(image, "get_data") and hasattr(brain_mask, "get_data") and not use_fcm:
        image = nibabel_to_ants(image)
        brain_mask = nibabel_to_ants(brain_mask)
    if not use_fcm:
        res = image.kmeans_segmentation(3, kmask=brain_mask, mrf=mrf)
        avg_intensity = [
            np.mean(image.numpy()[prob_img.numpy() > 0.5])
            for prob_img in res["probabilityimages"]
        ]
        csf_arg = (
            np.argmin(avg_intensity)
            if contrast.lower() in ("t1", "flair")
            else np.argmax(avg_intensity)
        )
        csf = res["probabilityimages"][csf_arg].numpy()
        if not return_prob:
            csf = (csf > csf_thresh).astype(np.float32)
    else:
        if hasattr(image, "numpy") and hasattr(brain_mask, "numpy"):
            image = to_nibabel(image)
            brain_mask = to_nibabel(brain_mask)
        seg = mask.fcm_class_mask(image, brain_mask, hard_seg=True)
        avg_intensity = [np.mean(image.get_fdata()[seg == i]) for i in range(1, 4)]
        csf_arg = (
            np.argmin(avg_intensity)
            if contrast.lower() in ("t1", "flair")
            else np.argmax(avg_intensity)
...
def csf_mask_intersection(image_dir, masks=None, prob=1):
    """
    ...
    for i, (img, mask) in enumerate(zip(data, masks)):
        ...
        logger.info(
            "Creating CSF mask for image {} ({:d}/{:d})".format(base, i + 1, len(data))
        )
...
def to_nibabel(image):
    ...
    affine = np.hstack(
        [image.direction * np.diag(image.spacing), np.array(image.origin).reshape(3, 1)]
    )
