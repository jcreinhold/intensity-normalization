...
def fcm_class_mask(image, brain_mask=None, hard_seg=False):
    """
    creates a mask of tissue classes for a target brain with fuzzy c-means

    Args:
        image (nibabel.nifti1.Nifti1Image): target image (must be T1w)
    ...
    if brain_mask is not None:
        mask_data = brain_mask.get_fdata() > 0.0
    else:
        estimated_foreground = data > data.mean()  # noqa
        mask_data = estimated_foreground
    mask_size = mask_data.sum()  # noqa
    foreground = data[mask_data]  # noqa
    ...
    shape = data.shape  # noqa
    mask = np.zeros(shape + (3,))
    for i in range(3):
        mask[..., i][mask_data] = t1_mem_list[i]
    if hard_seg:
        tmp_mask = np.zeros(shape)
        masked = mask[mask_data]
        tmp_mask[mask_data] = np.argmax(masked, axis=1) + 1
        mask = tmp_mask
    return mask


def gmm_class_mask(
    image, brain_mask=None, modality="t1", return_wm_peak=True, hard_seg=False
):
    """
    get a tissue class mask using gmms (or just the WM peak, for legacy use)

    Args:
        image (nibabel.nifti1.Nifti1Image): MR image
        brain_mask (nibabel.nifti1.Nifti1Image): brain mask for image
            (none if already skull-stripped)
        modality (str): string to describe image's MR modality
    ...
    brain = data[mask_data]  # noqa
    brain_vec = np.expand_dims(brain.flatten(), 1)
    ...
    if return_wm_peak:
        ...
        shape = data.shape  # noqa
        if hard_seg:
            tmp_predicted = gmm.predict(brain_vec)
            ...
            predicted_proba = gmm.predict_proba(brain_vec)
            mask = np.zeros((*shape, 3))
            for i, c in enumerate(classes):
...
def background_mask(img, seed=0):
    ...
    filled_closed_mask = __fill_2p5d(
        np.logical_or(closed_mask, filled_raw_mask)
    ).astype(np.float32)
