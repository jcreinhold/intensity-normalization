...
def preprocess(
    ...
    for i, (img_fn, mask_fn) in enumerate(zip(img_fns, mask_fns), 1):
        ...
        logger.info(
            "Preprocessing image: {} ({:d}/{:d})".format(img_base, i, len(img_fns))
        )
        ...
        if mask_dir is not None:
            ...
            img = ants.n4_bias_field_correction(
                img, convergence=n4_opts, weight_mask=smoothed_mask
            )
            if res is not None:
                if res != img.spacing:
                    mask = ants.resample_image(mask, res, False, 1)
            mask = (
                mask.reorient_image2(orientation)
                if hasattr(img, "reorient_image2")
                else mask.reorient_image((1, 0, 0))["reoimage"]
            )
        ...
        logger.info(
            "Writing preprocessed image: {} ({:d}/{:d})".format(
