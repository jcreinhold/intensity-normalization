...
def all_hists(image_dir, mask_dir=None, alpha=0.8, figsize=(12, 10), **kwargs):
    """
...
def hist(image, mask=None, ax=None, n_bins=200, log=True, alpha=0.8, lw=3, **kwargs):
    """
    plots the histogram of an ants object (line histogram) within a given brain mask
    or estimated foreground mask (the estimate is just all intensities above the mean)

    Args:
        image (nibabel.nifti1.Nifti1Image): MR image of interest
    ...
    if ax is None:
        _, ax = plt.subplots()
    data = (
        image.get_fdata()[mask.get_fdata() == 1]
        if mask is not None
        else image.get_fdata()
    )
