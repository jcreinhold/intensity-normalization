...
class TestNormalization(unittest.TestCase):
    ...
    def test_zscore_normalization(self):
        ...
        self.assertAlmostEqual(
            np.mean(normalized.get_fdata()[self.brain_mask.get_fdata() > 0]),
            0,
            places=4,
        )
    ...
    def test_fcm_normalization(self):
        ...
        self.assertAlmostEqual(
            normalized.get_fdata()[self.wm_mask.get_fdata() > 0.0].mean(),
            self.norm_val,
            places=3,
        )
    ...
    def test_gmm_normalization(self):
        ...
        normalized = gmm.gmm_normalize(
            self.img, self.brain_mask, norm_value=self.norm_val
        )
        self.assertAlmostEqual(
            normalized.get_fdata()[self.wm_mask.get_fdata() > 0.0].mean(),
            self.norm_val,
    ...
    def test_kde_normalization(self):
        ...
        normalized = kde.kde_normalize(
            self.img, self.brain_mask, modality="T1", norm_value=self.norm_val
        )
        self.assertAlmostEqual(
            normalized.get_fdata()[self.wm_mask.get_fdata() > 0.0].mean(),
            self.norm_val,
    ...
    def test_ws_normalization(self):
        ...
        normalized = whitestripe.ws_normalize(
            self.data_dir, "T1", mask_dir=self.mask_dir, write_to_disk=False
        )
        self.assertEqual(
            np.sum(normalized.get_fdata().shape), np.sum(self.img.get_fdata().shape)
        )
    ...
    def test_ravel_normalization(self):
        ...
        normalized = ravel_normalize(
            self.data_dir, self.mask_dir, "T1", write_to_disk=False
        )
    ...
    def test_ravel_normalization_csf_masks(self):
        ...
        normalized = ravel_normalize(
            self.data_dir, self.mask_dir, "T1", write_to_disk=False, csf_masks=True
        )
